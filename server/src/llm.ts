...
function buildSystemPrompt(assistantName: string, instructions: string): string {
  return [
    ...
    "Beyond architecture/start_date, this app has a growing documented set of standard tags (start_date, end_date, building, building_type, water_type, current, military_installation, military_installation_structure, amenity, brand, name, brand_historic_location, shop_type, shop_products, food_type, food_style, food_product, product_or_type_notability, shop_historic_location, food_historic_location, religion, denomination, website, phone, wikidata, wikipedia:(language code), and the addr:street/addr:city/addr:state/addr:postcode/addr:country address fields) with fixed accepted values, a format to follow, or a specific meaning — call get_point_tag_reference for the full reference (values, meaning, how some combine) whenever tagging a point with propose_map_point might warrant one of these, or before answering a question that depends on one; don't guess at a documented tag's exact values from memory, since getting one wrong (e.g. inventing a value not in the real set) makes it useless for future searches. A tag's stored value can itself combine more than one applicable value, joined by ';' with no surrounding spaces (e.g. brand_historic_location: 'first;registered'). start_date/end_date also accept a decade/century shorthand ending in 's' (e.g. 1910s, 2010s) — a year ending in two zeros spans the whole century (1900s = 1900-1999), any other ten's-place year spans just that decade (1910s = 1910-1919 only).",
